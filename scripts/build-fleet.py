...
<p>{esc(s['description'])}</p>{code}<div class="fleet-actions"><a href="{esc(s['image'])}" download="{Path(urlsplit(s['image']).path).name}">Скачать PNG ↓</a></div></div>{variants}</article>'''
...
for n, p in enumerate(data['planets'], 1):
    group = [s for s in active if s['planet'] == p['name']]
    content += f'''<section class="fleet-world" id="{p['id']}"><div class="fleet-heading"><div><div class="kicker">{['mercury','venus','earth','mars','jupiter','saturn','uranus','neptune'].index(p['id']) + 1:02d} / {p['ru']} · Аппаратов: {len(group)}</div><h2>{p['name']}</h2><p>{p['tagline']}</p></div><a href="index.html#{p['id']}">К главе →</a></div><div class="fleet-grid">'''
