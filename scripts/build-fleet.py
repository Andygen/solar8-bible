...
for s in ships:
    ...
    for variant in s.get('variants', []):
        assert (ROOT / urlsplit(variant['image']).path).is_file(), variant['image']
...
<p>{esc(s['description'])}</p>{code}<div class="fleet-actions"><a href="{esc(s['image'])}" download="{Path(urlsplit(s['image']).path).name}">Скачать PNG ↓</a>{detail_link}</div></div>{variants}</article>'''
...
for p in categories:
    ...
    preview += '</div></div>'
    if p.get('homeAnchor'):
        home.find(id=p['homeAnchor']).find_parent(class_='head').insert_after(parse(preview))
    else:
...
nav += ''.join(f'<a href="#{p["id"]}">{p["name"]} <span>Бонусная миссия</span></a>' for p in data.get('extras', []))
nav += '<div class="nav-group">МАТЕРИАЛЫ</div><a href="#fleet-rules">Названия и способности</a><a href="#archive">Архив концептов</a>'
content = f'''<section class="fleet-intro" id="overview"><div class="kicker">SOLAR 8 / КАТАЛОГ КОРАБЛЕЙ</div><h1>Флот</h1><p>От солнечной обороны Mercury до чужой флотилии Neptune. Корабли, объекты, их роли и оригинальные изображения.</p><div class="fleet-stats"><span><b>{len(active):02d}</b> кораблей и объектов</span><span><b>{len(data['planets']):02d}</b> планет</span></div><nav class="fleet-world-links" aria-label="Выбор планеты">'''
content += ''.join(f'<a href="#{p["id"]}">{p["name"]} <span>↘</span></a>' for p in categories) + '</nav></section>'
...
for p in categories:
    ...
    if p.get('note'):
        content += f'<p class="fleet-world-note">{esc(p["note"])}</p>'
    content += '</section>'
content += '<section class="fleet-rules" id="fleet-rules"><h2>Названия и способности</h2><p><a href="story-rules.html#fleet-map">Корпуса, боевые варианты и миссии появления →</a> · <a href="story-rules.html#assets">Недостающие материалы →</a></p>' + ''.join(f'<p>{esc(p)}</p>' for p in data['notes']) + '</section>'
...
for id in ['overview', 'fleet-rules', 'archive'] + [p['id'] for p in categories]:
